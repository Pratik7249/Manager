...
export default function TaskCard({ task }) {
  ...
  if (task.dueDate) {
    const date = parseISO(task.dueDate); // Use parseISO to handle ISO 8601 strings
    if (isPast(date) && !task.completed) {
      dueDateInfo = <p style={{ color: "red", margin: "5px 0" }}>Overdue</p>;
    } else if (!task.completed) {
      dueDateInfo = <p style={{ color: "#555", margin: "5px 0" }}>Due {formatDistanceToNow(date, { addSuffix: true })}</p>;
    }
  }

  return (
    <div style={{ border: "1px solid #ccc", borderRadius: 8, padding: 15, marginBottom: 10, backgroundColor: task.completed ? "#e9fde9" : "#fff" }}>
      <h3 style={{ marginTop: 0, textDecoration: task.completed ? "line-through" : "none" }}>
        {task.title}
      </h3>
